...
void cache_sim_t::init()
{
  ...
  bytes_written = 0;
  writebacks = 0;
  //initialize lfu_index
  for (size_t i = 0; i < sets; i++){
    std::vector<std::vector<size_t>> row;
    for (size_t j = 0; j < ways; j++){
      std::vector<size_t> value_count;
      value_count.push_back(j);//Record way
      value_count.push_back(0);//Original frequency
      row.push_back(value_count);
    }
    lfu_index.push_back(row);
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      lfu_index[idx][i][1] = lfu_index[idx][i][1]+1;//count +1
      //Reorder lfu_index
      size_t replace_idx = i;
      for (size_t j = i; j < ways; j++){
        if(lfu_index[idx][j][1] <= lfu_index[idx][i][1]){
          replace_idx = j;
        }
        else{
          break;
      ...
      for (size_t k = i; k < replace_idx; k++){
        size_t switch_way = lfu_index[idx][k][0];
        size_t switch_count = lfu_index[idx][k][1];
        lfu_index[idx][k][0] = lfu_index[idx][k+1][0];
        lfu_index[idx][k][1] = lfu_index[idx][k+1][1];
        lfu_index[idx][k+1][0] = switch_way;
        lfu_index[idx][k+1][1] = switch_count;
      }
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way = lfu_index[idx][0][0];//First way in index
  
  lfu_index[idx][0][1] = 0;//count = 0
  //Reorder lfu_index
  size_t replace_idx = 0;
  for (size_t j = 0; j < ways; j++){
    if(lfu_index[idx][j][1] <= lfu_index[idx][0][1]){
      replace_idx = j;
    }
    else{
      break;
  ...
  for (size_t k = 0; k < replace_idx; k++){
    size_t switch_way = lfu_index[idx][k][0];
    size_t switch_count = lfu_index[idx][k][1];
    lfu_index[idx][k][0] = lfu_index[idx][k+1][0];
    lfu_index[idx][k][1] = lfu_index[idx][k+1][1];
