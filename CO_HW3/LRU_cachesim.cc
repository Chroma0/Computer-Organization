...
void cache_sim_t::init()
{
  ...
  bytes_written = 0;
  writebacks = 0;
  //initialize lru_index
  for (size_t i = 0; i < sets; i++){
    std::vector<size_t> row;
    for (size_t j = 0; j < ways; j++){
      row.push_back(j);//original order
    }
    lru_index.push_back(row);
  }
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
        ...
        size_t replace = i;
        for (size_t j = 0; j < ways; j++){
          if(lru_index[idx][j] == i){
            for (size_t k = j; k < ways-1; k++){
              size_t switch_num = lru_index[idx][k];
              lru_index[idx][k] = lru_index[idx][k+1];
              lru_index[idx][k+1] = switch_num;
            }
            break;
          }
        }
...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way = lru_index[idx][0];//Least Recently Uesd number
  //Reorder lru_index
  for (size_t k = 0; k < ways-1; k++){
      size_t switch_num = lru_index[idx][k];
      lru_index[idx][k] = lru_index[idx][k+1];
      lru_index[idx][k+1] = switch_num;
  }  
